        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {/* Payment and Cancellation Buttons with status checks */}
              
              {/* 1. Show 'Pay Online' only if NOT cancelled, NOT paid, and NOT completed */}
              ...
              {/* 2. Show 'Paid' if NOT cancelled and payment IS done */}
              {!item.cancelled && item.payment && !item.isCompleted && (
                 <button className='sm:min-w-48 py-2 border rounded text-stone-500 bg-indigo-50'>Paid</button>
              )}

              {/* 3. Show 'Completed' only if the doctor marked it finished */}
              {item.isCompleted && (
                 <button className='sm:min-w-48 py-2 border border-green-500 rounded text-green-500'>Completed</button>
              )}

              {/* 4. Show 'Cancelled' status */}
              {item.cancelled && (
                 <button className='sm:min-w-48 py-2 border rounded text-red-500 border-red-500'>Appointment cancelled</button>
              )}

              {/* 5. Show 'Cancel' button only if NOT cancelled and NOT completed */}
